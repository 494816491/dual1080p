...
static pixel_type_t *p_pixel_type;
...
static u8* set_outline(const u32 outline_width, int* width, int* height)
{
	...
	if (FT_Get_Glyph(slot, &glyph) == 0) {
		...
		if (outlinespans->next != NULL) {
			...
			memset(pixel, p_pixel_type->pixel_background, pwidth * pheight *sizeof(u8));
								  ...
								  bitmap_info_t *bmp_info)
{
	...
	if (!cfg_font.disable_anti_alias
		...
		bmp_pitch = face->glyph->bitmap.pitch;//slot->bitmap.pitch;
	}

	if (bmp_pitch > buf_pitch || height > buf_height) {
		goto ERROR_QUIT;
	}
	if ((bmp_pitch > 0) && (bmp_addr != NULL)) {
		...
		for (row = 0; row < height; row++) {
			...
			for (col = 0; (col < bmp_pitch) && (col + offset_x) < buf_pitch;
				...
				if (cfg_font.disable_anti_alias)
					*ptr = p_pixel_type->pixel_font;
				else
					*ptr = p_pixel_type->pixel_font;//mapped_pixel[src_index];
				//printf("%d\t", *ptr);
				printf("%02x ", *ptr);
			}
			 printf("\n");
